function FoldText()
    ...
    local padding = vim.fn.winwidth(0) - string.len(countLines)
    ...
    if string.len(firstLine) > padding then
        local elipsis = "... "
        padding = padding - string.len(elipsis)
        firstLine = string.sub(firstLine, 1, padding) .. elipsis
    end

    -- Adjust the padding depending on the size of the first line.
    padding = padding - string.len(firstLine)
